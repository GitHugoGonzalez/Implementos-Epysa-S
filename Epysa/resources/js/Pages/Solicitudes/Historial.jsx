...
export default function Historial() {
  // 👇 defaults a prueba de nulos para no crashear
  const {
    solicitudes = { data: [], links: [], from: 0, to: 0, total: 0 },
    filtros = {},
    sucursales = [],
    // estados = [], // si no los usas, omítelos
  } = usePage().props;
  ...
  const resetFilters = () => {
    ...
    setEstado("");
    setSucursal("");
    router.get(route("solicitudes.historial"), {}, { preserveState: false, replace: true });
  };
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-7xl mx-auto bg-white shadow rounded-2xl p-6 mt-7">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
          ...
          {/* Si no usas 'estado' en el backend, puedes remover este select */}
          {/* <div>
            <select
              value={estado || ""}
              onChange={(e) => { setEstado(e.target.value); submitFilters({ replace:true, preserveState:true }); }}
              className="w-full border rounded-xl px-3 py-2"
            >
        ...
        <div className="flex items-center gap-2 mb-4">
          <button
            onClick={() => submitFilters({ replace: true, preserveState: true })}
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              ...
              {(solicitudes.data || []).map((s) => {
                const fecha = s.fecha_sol ? new Date(s.fecha_sol).toLocaleString() : "—";
                const estadoTexto = s.estado?.desc_estado ?? "—";
                const sucursalTexto = [s.sucursal?.ciudad, s.sucursal?.direccion].filter(Boolean).join(" - ") || "—";
                const solicitante = s.usuario?.name ?? "—";
                const detalle = (s.insumo?.nombre_insumo ?? "—") + (s.cantidad ? ` x ${s.cantidad}` : "");

                return (
                  <tr key={s.id_solicitud ?? `${s.fecha_sol}-${Math.random()}`} className="border-t hover:bg-gray-50">

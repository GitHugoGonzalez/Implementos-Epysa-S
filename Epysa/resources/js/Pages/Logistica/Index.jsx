            ...
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50 text-gray-700">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left">Estado Solicitud</th>
                  <th className="px-4 py-2 text-left">Estado Logística</th>
              ...
              <tbody>
                ...
                {solicitudes.map((s) => (
                  <tr key={s.id_solicitud} className="border-t">
                    ...
                    <td className="px-4 py-2">{s.estado_solicitud ?? "—"}</td>
                    <td className="px-4 py-2 capitalize">
                      {s.estado_logistica ?? "—"}
                    </td>
                    <td className="px-4 py-2">{fmtDate(s.fecha_envio)}</td>
                    <td className="px-4 py-2">{fmtDate(s.fecha_estimada)}</td>

                    <td className="px-4 py-2 text-right">
                      <Link
                        href={route("sol.logistica.edit", s.id_solicitud)}

    ...
    public function store(Request $request)
{
    ...
    if ($rol === 'operario') {
        ...
        $estadoInicial = $idAprobEncargado; // directo al jefe
    } elseif ($rol === 'jefe') {
        $estadoInicial = $idAprobJefe; // directo a logística
    } else {
    ...
    Solicitud::on('newdb')->create([
        ...
        'sucursal_nombre' => $conn->table('sucursal')->where('id_sucursal', $data['id_sucursal'])->value(DB::raw("CONCAT(ciudad,' — ',direccion)")),
        'id_insumo'       => $data['id_insumo'],
        'insumo_nombre'   => $conn->table('Insumos')->where('id_insumo', $data['id_insumo'])->value('nombre_insumo'),
